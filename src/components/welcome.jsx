...
const WelcomePage = () => {
  ...
  const { user, token, saveToken, refreshUser } = useAuth();
  ...
  React.useEffect(() => {
    ...
    if (tokenFromQuery) {
      saveToken(tokenFromQuery, true);
      refreshUser().then((fetched) => {
        if (!(fetched?.role)) {
  ...
  }, [navigate, refreshUser, saveToken, user]);
  ...
  const animateTo = React.useCallback((toValues, durationMs = 1200) => {
    cancelAnimationFrame(animRafRef.current); // stop animation first and than reload
    const from = metricValues.slice();// current values as starting point
  ...
  React.useEffect(() => {
    if (metricsInView && lastTargetRef.current !== 'target') { // avoid re-animating if already at target
  ...
  React.useEffect(() => {
    if (!metricsInView && lastTargetRef.current !== 'zero') {// avoid re-animating if already at zero
  ...
  React.useEffect(() => {
    const onScroll = () => {
      const el = metricsRef.current;
      if (!el) return;
      const rect = el.getBoundingClientRect();//ya element ke length, h , w ,position stor karta hai
  ...
  const chooseRole = async (role) => {
    if (!token) {
      setRoleError("Please log in before selecting a role.");
      return;
    }
    try {
      setRoleError("");
    ...
    <>
      ...
      {rolePromptOpen && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full p-6">
            ...
            {roleError && <p className="text-sm text-red-600 mt-4">{roleError}</p>}
          </div>
        </div>
      )}
      {/* Hero Section */}
      <div className="container mx-auto px-4 pt-36 pb-16 md:pt-65 md:pb-24 flex flex-col items-center text-center">
        <div className="max-w-4xl mx-auto">
          ...
          <h1 className="text-3xl md:text-7xl font-black  mt-15 text-gray-900 transform  uppercase tracking-wider">
            <span className="text-black">Find Your Dream Job</span>
            <br />
            <span className="text-[#6c6c71] "> in Pakistan</span>
          </h1>
