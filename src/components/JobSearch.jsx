...
const JobSearch = () => {
  ...
  const userKey = useMemo(() => (userId || userEmail || "anon").toString(), [userEmail, userId]);
  ...
  const toggleSaveJob = async (job) => {
    ...
    if (!userEmail) {
      showToast("Please login to save jobs", "warn");
      return;
    }
    ...
    try {
      if (isCurrentlySaved) {
        // Unsave job - call DELETE API
        const res = await axios.delete("/api/saved-jobs", {
        ...
        // Save job - call POST API
        const res = await axios.post("/api/saved-jobs", {
          userEmail,
          jobId
        });
      ...
      const errorMsg = error?.response?.data?.error || error?.message || "Failed to update saved job";
  ...
  const submitTest = async () => {
    ...
    const currentUserId = userId || userEmail || "anonymous";
    const applicantEmail = userEmail || undefined;
  ...
  const loadSavedJobs = useCallback(async () => {
    ...
    try {
      const res = await axios.get(`/api/saved-jobs?userEmail=${encodeURIComponent(userEmail)}`);
      const savedList = Array.isArray(res.data) ? res.data : [];
      const savedJobIds = new Set(savedList.map(item => String(item.jobId?._id || item.jobId || item.jobId?.id || "")));
