...
const Candidate = () => {
  ...
  const [applicationsRefreshTick, setApplicationsRefreshTick] = React.useState(0);
  ...
  React.useEffect(() => {
    ...
    const loadForSeeker = async () => {
      try {
        ...
        try {
          const savedRes = await fetch(`/api/saved-jobs?userEmail=${encodeURIComponent(email)}`);
          const savedList = await savedRes.json().catch(() => []);
          if (!ignore) setKpi4(Array.isArray(savedList) ? savedList.length : 0);
        } catch (e) {
        ...
        try {
          const res = await fetch(`/api/interviews/by-candidate/${encodeURIComponent(email)}`);
          const list = await res.json().catch(() => []);
          if (!ignore) setUpcoming(Array.isArray(list) ? list : []);
        } catch (e) {
          console.error("Error fetching interviews:", e);
          if (!ignore) setUpcoming([]);
        }

        try {
          const appsRes = await fetch(`/api/applications/by-user?email=${encodeURIComponent(email)}`);
          const apps = await appsRes.json().catch(() => []);
          const appsArray = Array.isArray(apps) ? apps : [];
          if (!ignore) {
            setAppliedJobs(appsArray);
            setKpi1(appsArray.length);
          }
        } catch (e) {
        ...
        console.error("Seeker load error:", error);
        if (!ignore) {
          setKpi1(0);
          setKpi2(0);
          setKpi3(0);
    ...
    return () => { ignore = true; };
  }, [userRole, email, location.pathname, applicationsRefreshTick]);

  React.useEffect(() => {
    const handler = () => setApplicationsRefreshTick((tick) => tick + 1);
  ...
  return (
    <div className="min-h-screen bg-gray">
      <div>
        {/* Sidebar */}
        <aside className="hidden md:flex md:fixed md:left-0 md:top-20 md:w-64 h-[calc(100vh-5rem)] overflow-auto flex-col bg-[black] space-y-6  z-40">
  ...
  <div className="fixed inset-0 bg-black/20 backdrop-blur-sm z-50 overflow-auto" onClick={()=> setShowSaved(false)}>
    <div className="p-6 flex justify-center">
      <div onClick={(e)=> e.stopPropagation()} className="w-full max-w-5xl">
        <SavedJobs userEmail={email} onChange={(count) => setKpi4(count)} />
      </div>
    </div>
  </div>
)}
