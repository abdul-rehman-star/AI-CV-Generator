...
const ProfileManagement = () => {
  ...
  // Load profile from API and localStorage
  React.useEffect(() => {
    const loadProfile = async () => {
      const email = userEmail || "";
      
      if (!email) {
        // If no email, just load from localStorage draft
        try {
      ...
      try {
        ...
        if (response.ok) {
          ...
          // Convert workExperience array to formData format
          const workExp = profile.workExperience && profile.workExperience.length > 0 
            ? profile.workExperience[profile.workExperience.length - 1] 
            : { company: "", role: "", years: "" };
          
          setFormData({
            personalInfo: profile.personalInfo || { name: "", email: "", phone: "" },
            education: profile.education || { degree: "", institution: "", year: "" },
            workExperience: workExp,
            workExperienceList: profile.workExperience || [],
            skills: profile.skills || { skillset: "", certifications: "" },
          });
      ...
      if (name || email) {
        setFormData((d) => ({ 
          ...
          personalInfo: { 
            ...d.personalInfo, 
            name: d.personalInfo.name || name, 
            email: d.personalInfo.email || email 
          } 
        }));
      }
    };
  ...
  const steps = [
    ...
    { id: 2, name: "Education Detail", icon: FaGraduationCap },
  ...
  // Navigation handlers
  const nextStep = React.useCallback(() => setStep((prev) => Math.min(prev + 1, steps.length)), [steps.length]);
  const prevStep = React.useCallback(() => setStep((prev) => Math.max(prev - 1, 1)), []);
  ...
  const saveAsDraft = async () => {
    try {
      ...
      // Also try to save to API if email is available
      const email = formData.personalInfo.email || userEmail || "";
      if (email && formData.personalInfo.name && formData.personalInfo.phone) {
        const profileData = {
          email: email,
          personalInfo: formData.personalInfo,
          education : formData.education,
          workExperience: formData.workExperienceList || (formData.workExperience ? [formData.workExperience] : []),
        ...
        try {
          ...
          console.log("API save failed, but draft saved locally:", apiError);
        }
      }
      ...
      window.appToast && window.appToast("Failed to save draft", "error");
    }
  };
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (!email) {
        window.appToast && window.appToast("Email is required to save profile", "error");
        return;
      }
      ...
      localStorage.removeItem("profileDraft"); // Clear draft after successful save

      window.appToast && window.appToast("Profile saved successfully 🎉", "success");
      
      // Redirect to CV generator after save
      setTimeout(() => navigate("/cv-generator"), 500);
    } catch (e) {
      console.error("Failed to save profile", e);
      window.appToast && window.appToast(e?.message || "Failed to save profile. Please try again.", "error");
    }
  };
      ...
      <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-10 items-start">
        ...
        <div className="flex items-center justify-between mb-12 relative">
          ...
          {steps.map((item) => (
            ...
            >
              <div
                className={`w-11 h-11 flex items-center justify-center rounded-full font-bold text-xl transition-colors duration-300 ${
              ...
              <p
                className={`md:text-lg mt-4 sm:text-md text-[9px] text-center font-semibold transition-colors duration-300 ${
        ...
        <div className="hidden ml-[-40px] md:flex items-start justify-end">
          <img
            ...
            className="w-[620px] h-[650px] shadow object-cover"
          />
        </div>
      </div>
      {/* Footer */}
