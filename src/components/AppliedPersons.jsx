...
const AppliedPersons = ({ inline, onOpenPassed, jobIdFilter }) => {
    ...
    const run = async () => {
      try {
        ...
        if (jobIdFilter) {
          ...
          const myJobs = Array.isArray(jobsAll)
            ? jobsAll.filter((j) => (j.postedByEmail || "").toLowerCase() === companyId.toLowerCase())
            : [];
  ...
  const openResumeSafe = (resumeUrl) => {
    try {
      ...
      console.error("Unable to open resume", e);
  ...
  const submitSchedule = async (e) => {
    ...
    try {
      setScheduling(true);
      if (!companyId) throw new Error("Company email missing");
      const jobsRes = await fetch(`/api/jobs`);
      const jobsAll = await jobsRes.json().catch(() => []);
      ...
      try {
        ...
        console.warn("Failed to update interview counter", storageError);
      }
